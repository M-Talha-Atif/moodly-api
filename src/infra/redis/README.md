...
Connects to `REDIS_URL` (falls back to `redis://localhost:6379`), with `maxRetriesPerRequest: 3` (`REDIS_MAX_RETRIES_PER_REQUEST`) and capped backoff on reconnect (`REDIS_RETRY_BASE_DELAY_MS` * attempt, up to `REDIS_RETRY_MAX_DELAY_MS`), both in `redis.constants.ts`.
