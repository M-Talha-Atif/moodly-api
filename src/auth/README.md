...
- **Refresh**: `POST /v1/auth/refresh` verifies the refresh JWT, compares its hash against the stored one, and rotates both tokens (issues new ones, re-hashes and re-stores the new refresh token).
- **Google OAuth**: `GET /v1/auth/google` kicks off the Passport flow; `GET /v1/auth/google/callback` finds-or-creates a user by email and redirects to `FRONTEND_URL` with the `jwt` cookie set.
...
|---|---|---|---|
| POST | `/v1/auth/signup` | – | Register a new user (email, password, name) |
| POST | `/v1/auth/login` | – | Login, sets `jwt` httpOnly cookie, returns tokens |
| POST | `/v1/auth/refresh` | – | Verify refresh token, rotate access+refresh tokens |
| POST | `/v1/auth/logout` | – | Clear stored refresh-token hash |
| GET | `/v1/auth/google` | `AuthGuard('google')` | Start Google OAuth flow |
| GET | `/v1/auth/google/callback` | `AuthGuard('google')` | OAuth callback: find-or-create user, set cookie, redirect to `FRONTEND_URL` |
| GET | `/v1/auth/me` | `JwtCookieGuard` | Return the currently authenticated user |
