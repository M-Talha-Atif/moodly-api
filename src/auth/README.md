...
- **Signup/login**: bcrypt (cost 12) password hashing. On success, `generateTokens()` signs an access token (`{sub, email, role}`, secret `JWT_SECRET`) and a refresh token (`{sub}`, secret `JWT_REFRESH_SECRET`): both currently set to a **7-day** expiry. The refresh token is bcrypt-hashed and stored on the `User` row.
